...
namespace back_1_trimestre_2_daw.Controllers
{
    ...
    public class SesionController : ControllerBase
    {
        ...
        public SesionController()
        {
            ...
            if (OpinionesController.GetOpiniones().Count == 0){

                new OpinionesController();
        ...
        [HttpGet("pelicula/{idPelicula}/opinion")]
        public ActionResult<IEnumerable<object>> GetOpinion(int idPelicula)
        {
            var sesionesPorPelicula = sesiones
                .Where(s => s.Pelicula.Id_Pelicula == idPelicula)
                .Select(s => new
                {
                    Opiniones = s.Opiniones
                })
                .ToList();
        ...
        private static void InicializarSesiones()
        {
            ...
            var opiniones = OpinionesController.GetOpiniones();

            // Asociar películas, salas y horarios para crear sesiones
            if (peliculas.Count > 0 && salas.Count > 0 && horarios.Count > 0 && opiniones.Count > 0)
            {   
                sesiones.Add(new Sesion(1, peliculas[0], salas[0], horarios[0], opiniones[0]));
                sesiones.Add(new Sesion(2, peliculas[0], salas[0], horarios[0], opiniones[1]));
                sesiones.Add(new Sesion(3, peliculas[1], salas[0], horarios[0], opiniones[2]));
                sesiones.Add(new Sesion(4, peliculas[2], salas[0], horarios[0], opiniones[0]));
                sesiones.Add(new Sesion(4, peliculas[2], salas[0], horarios[0], opiniones[1]));
                sesiones.Add(new Sesion(4, peliculas[2], salas[0], horarios[0], opiniones[2]));
            }
        }
    }
}
